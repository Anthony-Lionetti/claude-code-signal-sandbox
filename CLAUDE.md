...
## Creating a New ICA Style Problem
...
- In-memory database/key-value store — Start with SET/GET/DELETE, then add filtered scans, TTL with timestamps, compression/decompression
- Banking/transaction system — Basic operations that scale to multiple transaction types
- Inventory management system — ADD, COPY, GET operations with increasing constraints
- File system or package manager — Simulated CRUD with evolving requirements
- Web crawler — Parse URLs, handle domain restrictions, then add multithreading
...
- Include tests for:
  ...
  - Edge cases (empty, zero, negative, etc.)
...
#### LEVEL_X.md Template (in the `code/levels/` folder)
...
When the user asks for a "live interview", or a "mock interview", YOU become the interviewer. Do NOT just generate files — run an interactive interview session.
